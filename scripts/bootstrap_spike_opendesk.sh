#!/usr/bin/env bash
# Install this app as a real ExApp into the throwaway measurement topology of phase 17
# (compose.spike-opendesk.yml, openDesk spike).
#
#   export HP_SHARED_KEY="$(openssl rand -hex 32)"
#   export SECRET_KEY_BASE="$(openssl rand -hex 32)"
#   docker compose -f compose.spike-opendesk.yml up -d --wait   # 1. stage A, waits healthy
#   bash scripts/bootstrap_spike_opendesk.sh                    # 2. AppAPI, daemon, ExApp
#   docker compose -f compose.spike-opendesk.yml down -v        # 3. throw it away
#
# THIS SCRIPT SERVES A THROWAWAY MEASUREMENT ENVIRONMENT AND NEVER AN INSTALLATION. It
# creates users, hands out app passwords and switches the bruteforce guard off. All three are
# fine on an instance that lives on loopback for the length of one phase and unacceptable
# anywhere else. The topology it drives is pinned to Nextcloud 33.0.7, the version openDesk
# 1.18.0 pins (D-02), because every reachability proof of this project so far stands on
# 34.0.x and a proof on the wrong major version is not a proof.
#
# It is a copy of scripts/bootstrap_exapp.sh with the topology constants changed and the
# branch for the public instance removed. The one behavioural difference is the mail step:
# the spike topology carries no GreenMail service, so HAS_MAIL_SERVER below is 0 and the
# three mail calls are skipped, in the same shape the removed branch skipped them.
#
# Step 2 builds the image and pushes it into the local registry itself, so the three commands
# above are the whole installation.
#
# Idempotent by design: every step checks first and skips what already exists, so a second
# run is a no-op apart from a fresh app password in .env.spike-opendesk. That file is
# git-ignored; .env.spike-opendesk.example documents the variable names.
#
# The development loop without a container image:
#
#   bash scripts/bootstrap_spike_opendesk.sh --manual
#
...
#
# This script only ever talks to the throwaway topology named above. It never stops a
# container of another topology in this repository and never destroys a volume, which is why
# no other file name and no "down" command appears anywhere below.
set -euo pipefail
...
for argument in "$@"; do
  case "${argument}" in
    ...
    *)
      echo "ERROR: unknown argument '${argument}'. Known flag: --manual." >&2
      exit 1
      ;;
  esac
done
...
# passwords and switches `auth.bruteforce.protection.enabled` off. All three are fine on the
# throwaway measurement topology of phase 17 and unacceptable on any other instance, so a
# forgotten `export COMPOSE_FILE=...` in the calling shell must not be able to aim it at the
# test topology or the ExApp topology of this repository, both of which are in daily use.
COMPOSE_FILE="compose.spike-opendesk.yml"
PROJECT_NAME="nc-mcp-spike-od"
SERVICE="nextcloud"
HARP_CONTAINER="${HARP_CONTAINER:-nc-mcp-spike-od-harp}"
HOST_PORT="${NC_SPIKE_OD_PORT:-8091}"
# The name of the compose network, as written in compose.spike-opendesk.yml. The deploy
# daemon attaches the ExApp container to it, which is how HaRP and Nextcloud reach the app,
# and it is the network the egress control measurement of D-06 resolves its neighbours in.
NETWORK_NAME="${NC_SPIKE_OD_NETWORK:-nc-mcp-spike-od-net}"
DAEMON_NAME="harp_proxy_docker"
MANUAL_DAEMON_NAME="manual_install"
APP_ID="mcp_connector"
APP_NAME="MCP Connector"
# Loopback registry from compose.spike-opendesk.yml. The Docker daemon of the host treats a
# 127.0.0.1 registry as an allowed insecure one without any extra configuration. Port 5001
# and not 5000, so this topology can stand next to compose.spike-opendesk.yml.
REGISTRY="${NC_SPIKE_OD_REGISTRY:-127.0.0.1:5001}"
IMAGE_NAME="${APP_ID}"
...
APP_PORT="${NC_SPIKE_OD_APP_PORT:-23000}"
MANUAL_APP_PORT="${NC_SPIKE_OD_MANUAL_PORT:-23001}"
...
ALICE_PASSWORD="${NC_SPIKE_OD_ALICE_PASSWORD:-alice-test-pw-01}"
BOB_PASSWORD="${NC_SPIKE_OD_BOB_PASSWORD:-bob-test-pw-01}"
...
# service of compose.spike-opendesk.yml, which lives in that throwaway topology only and accepts mail
...
ALICE_IMAP_PASSWORD="${NC_SPIKE_OD_ALICE_IMAP_PASSWORD:-alice-spike-imap-pw}"
# Whether this topology carries a mail server. It does not: compose.spike-opendesk.yml has
# no GreenMail service, because no measurement of phase 17 reads a mailbox and a service that
# accepts mail without authentication is not started for nothing. The three mail steps below
# are skipped in exactly the shape the removed public-instance branch skipped them, so the
# mail account of alice stays a row without a server behind it. Set to 1 only together with a
# GreenMail service in the compose file.
HAS_MAIL_SERVER=0
# The IMAP and SMTP endpoint that account would use. `greenmail` is the service name the
# ExApp topology uses; the constants stay so that the functions below need no edit if a
# later plan of this phase does add the service.
MAIL_HOST="greenmail"
MAIL_IMAP_PORT="3143"
MAIL_SMTP_PORT="3025"
TOKEN_NAME="mcp-exapp"
ENV_FILE="${ENV_FILE:-.env.spike-opendesk}"
...
# Extra arguments every `docker compose` call of this script carries. Empty: this topology
# needs no environment file, its two mandatory variables come from the environment (see the
# header). The array stays, because every compose call goes through dc() below and a later
# plan of this phase may need an --env-file for the OpenProject profile.
COMPOSE_ENV_ARGS=()
# Disabling the bruteforce guard is a property of an unreachable instance. This one publishes
# nothing outside 127.0.0.1 and is thrown away at the end of the phase.
...
PUBLIC_URL="${NC_SPIKE_OD_PUBLIC_URL:-${BASE_URL}/exapps/${APP_ID}}"
...
# through it, so an environment file can never be forgotten on one of them.
dc() {
...
deliver_test_mail() {
  ...
  if [ "${HAS_MAIL_SERVER}" -eq 0 ]; then
    # The spike topology carries no GreenMail service (see HAS_MAIL_SERVER above): no
    # measurement of phase 17 reads a mailbox. The mail account of alice stays a row without
    # a server behind it, exactly as it was before plan 10-01.
    echo "test mails ${uid}: skipped (this topology carries no mail server)"
    return 0
  fi
  MAIL_PW="${password}" \
  MAIL_HOST="${MAIL_HOST}" \
...
def plain(sender, subject, body):
    ...
    # (measured on 2026-08-24, the stack trace is in the log of nc-mcp-exapp-greenmail; this
    # topology skips the step, see HAS_MAIL_SERVER).
...
sync_mail_account() {
  local uid="$1" output
  if [ "${HAS_MAIL_SERVER}" -eq 0 ]; then
    echo "mail sync ${uid}: skipped (this topology carries no mail server)"
    return 0
  fi
...
# of them the current .env.spike-opendesk happens to name. Ten hex characters, so a marker can never
...
# placeholder from .env.spike-opendesk.example, and that file is published in git: whoever reads it
...
require_registry_shape() {
  ...
  if ! printf '%s' "$value" | grep -E '^[0-9A-Za-z_.:-]+$' >/dev/null; then
    echo "ERROR: NC_SPIKE_OD_REGISTRY is '${value}', which is not a host[:port] shape." >&2
...
# come from the environment, and the name is what the caller has to fix.
...
# to across every further run, and .env.spike-opendesk.example ships a placeholder that the obvious
# `cp .env.spike-opendesk.example .env.spike-opendesk` would turn into the real registration secret (CR-02).
app_secret() {
  local existing=""
  if [ -f "${ENV_FILE}" ]; then
...
ensure_daemon_harp() {
  ...
  # /exapps/, otherwise the heartbeat fails (research pitfall 7). A failing heartbeat is not
  # only a bug here, it is the S0 finding of OD-01; compose.spike-opendesk.yml gives Caddy the
...
echo "== phase 17 spike topology bootstrap =="
echo "topology: ${PROJECT_NAME} on ${BASE_URL}, throwaway measurement environment"
ensure_own_topology
require_port_number NC_SPIKE_OD_APP_PORT "${APP_PORT}"
require_port_number NC_SPIKE_OD_MANUAL_PORT "${MANUAL_APP_PORT}"
require_registry_shape "${REGISTRY}"
require_url_shape NC_SPIKE_OD_PUBLIC_URL "${PUBLIC_URL}"
wait_for_install
...
if [ "${HAS_MAIL_SERVER}" -eq 1 ]; then
  ...
  echo "mail account alice: skipped (this topology carries no mail server)"
fi
...
# `set -a && . ./.env.spike-opendesk`, and an unquoted value with a space in it makes the shell run its
...
# run and hand us random 429s (research pitfall 8). Throwaway instance on loopback only,
# never a recommendation for a real server.
...
#        docker compose -f compose.spike-opendesk.yml cp app_api nextcloud:/var/www/html/custom_apps/app_api
#        docker compose -f compose.spike-opendesk.yml exec -T --user root nextcloud \
...
# This script pins it: the value from .env.spike-opendesk is passed in the json-info payload, so a
# re-registration keeps the secret stable. If .env.spike-opendesk was deleted, a new secret is
# generated, and then everything that still holds the old one has to be restarted:
#
#   docker compose -f compose.spike-opendesk.yml exec -T --user www-data nextcloud \
#     php occ app_api:app:unregister mcp_connector
#   rm -f .env.spike-opendesk        # only if you want a new secret on purpose
