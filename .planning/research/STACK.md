...
**Domain:** Nextcloud MCP-only ExApp (Python MCP-Server, App-Store-Distribution via AppAPI)
**Researched:** 2026-08-14
**Confidence:** HIGH (Kernentscheidungen gegen PyPI und offizielle Doku verifiziert)

## Wichtigster Befund zuerst: Die SDK-Lage hat sich geaendert

PROJECT.md (Key Decision "Stabiler SDK-Stand 1.27 statt 2.0-Beta") basiert auf einer veralteten Praemisse:
...
**Empfehlung: mcp>=2.0,<3 statt 1.27.** Ein Neuprojekt mit Launch September 2026 auf einer Maintenance-only-Linie zu starten waere technische Schuld ab Tag 1. Das Remote-Requirement "stateless-ready" ist in v2 nativ (jede Replica hinter Round-Robin-LB, keine Sticky Sessions). Fallback-Plan: bei einem Blocker auf `mcp>=1.29,<2` pinnen; die Architektur (freistehende Tool-Funktionen, kein Session-State) bleibt identisch.
...
uv init --python 3.13
uv add "mcp[cli]>=2.0,<3" "nc_py_api[app,calendar]>=0.30,<1" \
       httpx lxml icalendar recurring-ical-events vobject "pyjwt[crypto]"
...
**Remote-Modus (Streamable HTTP, Kern-Szenario):**
...
---
*Stack research for: Nextcloud MCP-only ExApp (Python)*
*Researched: 2026-08-14*
