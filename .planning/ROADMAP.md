...
**Success Criteria** (was wahr sein muss):
  ...
  2. Kein Produktionsmodul baut einen Id-String außerhalb des Codecs: `tools/chatgpt.py` ruft `ids.encode_mail`, der `_ID_KIND`-Workaround existiert nicht mehr, und `ids.parse` lehnt eine `url:`-Id mit Whitespace im Rest ab statt sie durchzureichen; für beide Wege existiert je ein Negativtest.
