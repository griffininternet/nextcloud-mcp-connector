...
key-decisions:
  ...
  - "KORRIGIERT (Review 04, BL-01): Der Widerruf der Seite gibt das App-Passwort sehr wohl an Nextcloud zurück, als letzter Schritt und best effort wie auf /revoke. Die ursprüngliche Entscheidung (nur note_cleanup, Rückgabe durch den Sweep) war nicht durchsetzbar: abandoned_authorizations filtert revoked_at IS NULL und kann eine gerade widerrufene Zeile nie sehen"
...
- **Kein Nextcloud-Aufruf im Seiten-Widerruf.** ~~`end_connection` gibt das App-Passwort nicht an Nextcloud zurück, sondern schreibt `note_cleanup`; das Zurückgeben erledigt `sweep_abandoned`.~~ **Korrigiert (Review 04, BL-01):** Diese Aussage war falsch, und zwar nicht in der Umsetzung, sondern in der Annahme. `sweep_abandoned` liest `abandoned_authorizations`, und das Statement filtert `a.revoked_at IS NULL`: eine gerade widerrufene Zeile ist für diesen Sweep unsichtbar. Der einzige weitere Pfad wäre `sweep_expired_clients` nach 90 Tagen Nichtbenutzung des Clients, was für einen benutzten Client nie eintritt. `end_connection` ruft deshalb jetzt `_hand_back`, als letzten Schritt und best effort, genau in der Reihenfolge von `_revoke`: erst widerrufen, dann Nextcloud fragen. Pitfall 13 bleibt gewahrt, weil ein Fehlschlag die Verbindung trotzdem beendet und `cleanup_at` stehen lässt.
...
| T-04-37 (Denial of Service, Formular-Flut) | Geschlossen, **korrigiert (Review 04, HI-01)**: die geteilte Klasse `CLASS_AUTHORIZE` war selbst die Lücke, weil 200 anonyme E8-Ablehnungen den Klassendeckel füllten und damit Notbremse und Consent-Oberfläche für alle schlossen. Die Route hat jetzt `CLASS_CONNECTIONS`, zählt pro signiertem Konto statt pro gefälschter Quelle und zählt anonyme Anfragen gar nicht. | `test_the_page_is_throttled_in_a_class_of_its_own`, `test_an_anonymous_flood_closes_neither_the_brake_nor_the_consent_surface`, `test_the_refusals_of_one_account_do_not_lock_out_another` |
...
**Neue Angriffsfläche, ausdrücklich gemeldet:** `/connections` ist die dreizehnte deklarierte Route und die zweite PUBLIC-Route, hinter der ein Formular Zustand ändert (die erste ist `/authorize/decide`). Sie ist ohne Nextcloud-Konto erreichbar und antwortet dann E8 mit 403; jede zustandsändernde Aktion verlangt zusätzlich einen Anti-Fälschungs-Wert, den nur diese Installation erzeugen kann, und einen Ownership-Vergleich gegen die HaRP-Identität. **Präzisiert (Review 04, LO-04):** Auf dieser Seite entsteht kein neuer Fall dieser Art, alle fünf Aktionen tragen den Anti-Fälschungs-Wert (`confirm` seit LO-04/LO-07 ebenfalls). Über `POST /connect` sagt das nichts: AR-03-08 bleibt unverändert offen, und der Satz "wiederholt sich hier nicht" bezog sich nur auf `/connections`, las sich im Zusammenhang aber wie eine Entlastung für beide.
...
- **Der Widerruf gibt das App-Passwort zurück.** ~~`note_cleanup` markiert es; `sweep_abandoned` gibt es zurück.~~ **Korrigiert (Review 04, BL-01):** `note_cleanup` markiert es, `_hand_back` gibt es unmittelbar danach zurück, und erst ein fehlgeschlagener Rückgabe-Versuch lässt die Markierung stehen. Für den Nutzer endet die Verbindung ohnehin sofort, weil jeder Token an der widerrufenen Autorisierung hängt; neu ist, dass der Eintrag auch unter "Geräte und Sitzungen" in Nextcloud verschwindet.
