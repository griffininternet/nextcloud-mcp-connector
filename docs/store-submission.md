...
| 2026-08-25 18:30Z | The second half of step 4 and all of step 5 are done for 0.1.9. The tag `v0.1.9` exists locally and on the remote and points at `685295d`, the commit `main` already carried, so nothing was tagged that the public branch does not serve. The release workflow of that tag is green in every step, run `32883904698`, job `publish` success in 1m40s: the multi arch image was built and pushed to `ghcr.io/street1983nk/mcp_connector:0.1.9`, and the store archive was built and attached to the GitHub release as `mcp_connector-0.1.9.tar.gz` with 47264 bytes. The tag came into being only after an explicit owner release at 18:27Z, and the commits of this phase were on the public `main` before it existed, which the row above records at 18:21Z. No tag was rewritten, no asset was deleted, and the 47264 bytes are the published size, not the 47546 of the local structure check | `git push origin main`, then `git tag v0.1.9` and `git push origin v0.1.9`, then `gh run watch 32883904698 --exit-status`, exit 0, and `gh run view 32883904698 --json conclusion` gives `success`. `git ls-remote --tags origin v0.1.9` names `685295d7d1e0ac227d6611d33fb3eb799351c800`, and `gh release view v0.1.9 --json assets` names `mcp_connector-0.1.9.tar.gz` |
